...
from utils.fuselage_drag_estimation import fuselage_drag
...
class WingGeometryConditions:
    ...
    def __init__(self, var_Sref, var_AR, var_lambda, var_Lambda, var_tc, var_h, var_M, var_WT, var_Theta, var_chord_kink=0, var_sweep_outboard=0, var_b_kink=0):
        ...
        def MAC(root_chord, taper):
            return 2 / 3 * root_chord * ((1 + taper + taper ** 2) / (1 + taper))
        ...
        if not (var_chord_kink == 0 and var_b_kink == 0 and var_sweep_outboard == 0):
            ...
            self.taper_kink = var_chord_kink

            self.b_outboard = self.half_span - self.b_kink

            self.root_chord = self.Sref / (self.b_kink*(1+var_chord_kink) + self.b_outboard*(var_chord_kink + self.taper))
            self.kink_chord = self.taper_kink*self.root_chord
            self.tip_chord = self.taper*self.root_chord

            S1 = self.b_kink * (self.root_chord + self.kink_chord)/2
            S2 = self.b_outboard * (self.kink_chord + self.tip_chord)/2
            taper_kt = self.taper/self.taper_kink

            MAC1 = MAC(self.root_chord, self.taper_kink)
            MAC2 = MAC(self.kink_chord, taper_kt)

            self.MAC = (MAC1*S1 + MAC2*S2)/(S1+S2)
            self.Swing_calculated = S1 + S2
            self.HasKink = True
        else:
            self.HasKink = False
            self.root_chord = 2 * self.Sref / (self.b * (1 + self.taper))
            self.tip_chord = self.root_chord * self.taper
            self.MAC = MAC(self.root_chord, self.taper)
            self.Swing_calculated = self.half_span*(self.root_chord + self.tip_chord)/2
        ...
        #Assume fuselage drag
        self.CD0_S = fuselage_drag(self.M, T, rho, a, l=20, d=3)
        self.CD0 = self.CD0_S/self.Sref
    ...
    def prepare_input(self, airfoils, path_title_dict):
        ...
        if all(airfoil[:4] =='naca' and len(airfoil) == 8 for airfoil in airfoils.values()):
            ...
            transform_airfoils(airfoils=airfoils,
                               ...
                               twist=self.twist,
                               kink_chord=self.kink_chord,
                               sweep_outboard=self.sweep_outboard,
                               b_kink=self.b_kink)

            root_airfoil_csv = os.path.join(airfoil_folder_path, 'root_airfoil.csv')
            kink_airfoil_csv = os.path.join(airfoil_folder_path, 'kink_airfoil.csv')
            ...
            kink_airfoil_wgs = wgs_creator.read_airfoil(kink_airfoil_csv, y_coordinate=self.b_kink)
        ...
        dist_in = self.b_kink
        dist_out = self.half_span - dist_in

        num_in = int(np.round(dist_in / grid_dist))
        num_out = int(np.round(dist_out / grid_dist))

        wing_inboard = root_airfoil_wgs.linspace(kink_airfoil_wgs, num=num_in) #num=20
        wing_outboard = kink_airfoil_wgs.linspace(tip_airfoil_wgs, num=num_out) #num=20

        wing = wing_inboard.concat_row(wing_outboard)
...
def run_tool(in_file, out_file):
    HF_tool_info = dict(
        ...
        inputs=[
            ...
            '/dataSchema/aircraft/geometry/chord_kink',
            '/dataSchema/aircraft/geometry/sweep_outboard',
            '/dataSchema/aircraft/geometry/b_kink',
    ...
    print(f'Area of single wing: {SSBJ_wing.Swing} / {SSBJ_wing.Swing_calculated} \n'
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        in_file = os.path.join(os.getcwd(), 'Error_inputs', 'CPACS_in.xml')
